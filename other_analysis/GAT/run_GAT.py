...
from utils import load_BioNet_data, EarlyStopping, index_generator, parse_minibatch, load_MDA_data
...
def run_model_GAT_lp(feats_type, hidden_dim, num_heads, num_epochs, patience, batch_size, repeat, save_postfix, metapath_0_idx, metapath_1_idx):
    ...
    for _ in range(repeat):
        ...
        net = GAT_lp(in_dims, hidden_dim, hidden_dim, hidden_dim, num_heads, dropout_rate)
        net.to(device)
        ...
        for epoch in range(num_epochs):
            ...
            #     #     adjlists_ua_1, train_pos_user_artist_batch, device, num_user)
            #     # train_neg_g_lists, train_neg_idx_batch_mapped_lists, train_neg_indices_lists = parse_minibatch(
            #     #     adjlists_ua_1, train_neg_user_artist_batch, device, num_user)
            ...
            #             adjlists_ua_1, val_pos_user_artist_batch, device, num_user)
            #         val_neg_g_lists, val_neg_idx_batch_mapped_lists, val_neg_indices_lists = parse_minibatch(
            #             adjlists_ua_1, val_neg_user_artist_batch, device, num_user)
            #
            #         pos_embedding_user, pos_embedding_artist = net(
        ...
        with torch.no_grad():
            for iteration in range(test_idx_generator.num_iterations()):
                ...
                test_pos_g_lists, test_pos_idx_batch_mapped_lists, test_pos_indices_lists = parse_minibatch(
                    adjlists_ua_1, test_pos_user_artist_batch, device, num_user)
                test_neg_g_lists, test_neg_idx_batch_mapped_lists, test_neg_indices_lists = parse_minibatch(
                    adjlists_ua_1, test_neg_user_artist_batch, device, num_user)
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='GAT testing for the biology dataset')
    ...
    ap.add_argument('--save-postfix', default='gat_bio',
                    help='Postfix for the saved model and result. Default is LastFM.')
    ap.add_argument('--metaclass-zero', type=int, default=1, help='instance indices of gene list.')
    ap.add_argument('--metaclass-one', type=int, default=0, help='instance indices of disease list.')

    args = ap.parse_args()
    run_model_GAT_lp(args.feats_type, args.hidden_dim, args.num_heads, args.epoch,
                     args.patience, args.batch_size, args.repeat, args.save_postfix, args.metaclass_zero,
                     args.metaclass_one)
