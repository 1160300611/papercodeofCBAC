...
#                 [[1, 0], [2, 3], [1, None, 0]]]
etypes_lists = [[[0, 1], [None]],
                [[2, 3], [4, 5]]]
use_masks = [[False, False],
             [True, False]]
...
def run_model_LastFM(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    adjlists_ua, edge_metapath_indices_list_ua, _, type_mask, train_val_test_pos_user_artist, train_val_test_neg_user_artist = load_BioNet_data()
    ...
    for _ in range(repeat):
        net = MAGNN_lp(
            [2, 2], 6, etypes_lists, in_dims, hidden_dim, hidden_dim, num_heads, attn_vec_dim, rnn_type, dropout_rate)
        net.to(device)
        ...
        for epoch in range(num_epochs):
            ...
            #     #     no_masks, num_user)
            #     train_g_lists, train_indices_lists, train_idx_batch_mapped_lists = parse_minibatch_LastFM(
            #         adjlists_ua, edge_metapath_indices_list_ua, train_batch, device, neighbor_samples,
            #         no_masks, num_user)
            #
            #     t1 = time.time()
            #     dur1.append(t1 - t0)
        ...
        with torch.no_grad():
            for iteration in range(test_idx_generator.num_iterations()):
                ...
                neg_embedding_user = neg_embedding_user.view(-1, 1, neg_embedding_user.shape[1])
                neg_embedding_artist = neg_embedding_artist.view(-1, neg_embedding_artist.shape[1], 1)
                gene_embedding_list.append(pos_embedding_user.cpu().numpy())
                dis_embedding_list.append(pos_embedding_artist.cpu().numpy())
        ...
        # np.savez('embedding2vis_InfoMaxFusv1.npz', gene=np.concatenate(gene_embedding_list, axis=0),
...
if __name__ == '__main__':
    ...
    ap.add_argument('--samples', type=int, default=200, help='Number of neighbors sampled. Default is 100.')
    ap.add_argument('--repeat', type=int, default=1, help='Repeat the training and testing for N times. Default is 1.')
    ap.add_argument('--save-postfix', default='LastFM',
